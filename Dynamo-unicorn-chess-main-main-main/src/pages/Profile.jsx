...
const Profile = () => {
    ...
    const historyurl = `${import.meta.env.VITE_URL}${
        import.meta.env.VITE_HISTORY
    }/${userId}`;
    const getWalletHistoryUrl = `${import.meta.env.VITE_URL}${
        import.meta.env.VITE_GET_WALLET_HISTORY
    }?page=1&limit=4`;

    const {
        data: queryGetPROFILE,
        isLoading,
        error,
        refetch,
    } = useQuery("getPROFILE", () => getApiWithToken(url));
    ...
    const handleSave = async () => {
        try {
            const response = await axios.post(
                "https://chess.${VITE_URL}/update-profile-user",
                {
                ...
                },
                {
                    headers: {
                        Authorization: JSON.parse(
                            localStorage.getItem("chess-user-token")
                        ),
    ...
    const queryGetWalletHistory = useQuery("GetWalletHistory", () =>
        getApiWithToken(getWalletHistoryUrl)
    );
    ...
    useEffect(() => {
        if (
            ...
            queryGetWalletHistory.isLoading
        ) {
            startLoading();
        } else {
            finishLoading();
    ...
    // Make sure logout button always appears for authenticated users
    ...
    const Logout = () => {
        ...
        localStorage.removeItem("device_id"); // Optional: clear deviceId
    ...
    const handleWithdrawal = async (amount) => {
        try {
            const url = `${import.meta.env.VITE_URL}${
                import.meta.env.VITE_GET_WALLET_WITHDRAW
            }`;
            const raw = {
                dynamoCoin: amount,
                type: "withdraw",
    ...
    const tournamentsuUrl = `${import.meta.env.VITE_URL}${
        import.meta.env.VITE_GET_MY_TOURNAMENT
    }`;
    const queryGetTournaments = useQuery("GetTournaments", () =>
        getApiWithToken(tournamentsuUrl)
    );
    const FORUSERUrl = `${import.meta.env.VITE_URL}${
        import.meta.env.VITE_GET_ORDER_FORUSER
    }`;
    const queryGetFORUSER = useQuery("GetFORUSE", () =>
        getApiWithToken(FORUSERUrl)
    );
    const orders = queryGetFORUSER?.data?.data?.orders;

    const handleDelete = async () => {
        try {
            const deleteUserUrl = `${import.meta.env.VITE_URL}${
                import.meta.env.VITE_DELETE_USER_BY_ID
            }${userId}`;
                    ...
                    <div className="flex flex-wrap -mx-3">
                        ...
                        <div className="flex-none w-auto max-w-full px-3 my-auto">
                            <div className="h-full">
                                <h5 className="mb-1 font-bold  text-lg  text-[#16884d]">
                        ...
                        <div className="w-full max-w-full px-3 mx-auto mt-4 sm:my-auto sm:mr-0 md:w-1/2 md:flex-none lg:w-4/12">
                            <div className="relative right-0">
                                <ul className="relative flex flex-wrap justify-end p-1 list-none bg-transparent rounded-xl">
                            ...
                            <div className="p-4 pb-0 mb-0 border-b-0 rounded-t-2xl">
                                <div className="flex flex-wrap -mx-3">
                                    ...
                                    <div className="w-full max-w-full px-3 text-right shrink-0 md:w-4/12 md:flex-none relative">
                                        ...
                                        {queryGetPROFILE?.data?.userStatus
                                            ?.status === "flag" && (
                            ...
                            <div className="flex-auto p-4">
                                ...
                                <ul className="flex flex-col pl-0 mb-0 rounded-lg">
                                    ...
                                    <li className="relative block px-4 py-2 pl-0 leading-normal border-0 border-t-0 text-base text-inherit">
                                        ...
                                        &nbsp;
                                        {queryGetPROFILE?.data?.Rating?.toFixed(
                                            2
                                        )}
                                    </li>
                                </ul>
                            ...
                            <div className=" p-4 w-full">
                                <ul className="overflow-y-auto  bg-white h-64  w-full pl-0  rounded-lg">
                                    {notificationFunData?.map((item, index) => (
                                        <li
                                            key={index}
                                            className="px-2 py-1 mb-2 w-full flex justify-between  border-0 rounded-md"
                                        >
                                            <div className="p-2">
                                            ...
                                            <p className="p-3">
                                                <button
                                                    onClick={(e) =>
                                                        handlejoin(e, item.url)
                                                    }
                                                    className=" bg-green-600 text-white rounded-sm p-1 text-sm"
                                                >
                                                    Join Room
                                                </button>
                                            </p>
                                        ...
                                        {queryGetTournaments?.data?.data?.data.map(
                                            (tournament, index) => (
                                                <div
                                                    onClick={() =>
                                                        navigate(
                                                            `/LiveTournamentDetail/${tournament._id}`
                                                        )
                                                    }
                                                    key={index}
                    ...
                    <div className="p-4 w-full">
                            ...
                            <div className="bg-white md:p-2 p-6 rounded-lg border border-gray-200 mb-4 lg:mb-0 shadow-md lg:w-[35%]">
                                <div className="flex justify-center items-center space-x-5 h-full">
                                    <div>
                                        ...
                                        <h2 className="text-4xl font-bold text-gray-600 flex">
                                            {queryGetPROFILE?.data?.dynamoCoin}{" "}
                                            <span className="flex">
                                                <img
                                                    className="w-12 h-10"
                                                    src={Coinicon}
                            ...
                            <div className="bg-white p-4 rounded-lg xs:mb-4 max-w-full shadow-md lg:w-[65%]">
                                <div className="flex flex-wrap justify-between h-full">
                                    <div
                                        ...
                                        className="flex-1 bg-gradient-to-r bg-green-600 rounded-lg flex flex-col items-center justify-center p-4 space-y-2 border border-gray-200 m-2"
                                    >
                                        <TiDownload size={30} />
                        ...
                        >
                            <table className="table-auto w-full">
                                ...
                                <tbody>
                                    {queryGetWalletHistory?.data?.data?.data.map(
                                        (transaction, index) => (
                                            <tr
                                                key={index}
                        ...
                        <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                            <div className="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
                                ...
                                <div className="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                                    <button
                                        onClick={() =>
                                            handleWithdrawal(amountWithdrawal)
                                        }
                                        type="button"
                                        className="inline-flex w-full justify-center rounded-md bg-green-600 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-green-500 sm:ml-3 sm:w-auto"
                                    >
                    ...
                    <table className="min-w-full">
                        ...
                        <tbody className="bg-gray-400">
                            {GameHistory?.data?.data?.data
                                ?.sort(
                                    (a, b) =>
                                        new Date(b.analysisData[0]?.createdAt) -
                                        new Date(a.analysisData[0]?.createdAt)
                                )
                                .map((game, index) => (
                                    <tr
                                        key={index}
                                    ...
                                    >
                                        ...
                                        <td className="py-3 px-4">
                                            {game.analysisData[0].DrawStatus
                                                ? "Draw"
                                                : game.analysisData[0]
                                                      .winner === userId
                                                ? "Win"
                                        ...
                                        <td className="py-3 px-4">
                                            <span
                                                onClick={() =>
                                                    navigate(
                                                        `/analysisBoard/${game.analysisData[0]._id}`
                                                    )
                                                }
